...
int tokenize(const char *expr, Token *tokens, int max_tokens) {
    ...
    while (pos < len && token_count < max_tokens) {
        ...
        if (isdigit(expr[pos]) || expr[pos] == '.') {
            start = pos;
            while (pos < len && (isdigit(expr[pos]) || expr[pos] == '.')) pos++;
            substr_len = pos - start;
            if (substr_len >= sizeof(tokens[token_count].value)) {
                fprintf(stderr, "Error: Number token too long.\n");
                return -1;
            }
            strncpy(tokens[token_count].value, expr + start, substr_len);
            tokens[token_count].value[substr_len] = '\0';
        ...
        if (isalpha(expr[pos])) {
            ...
            substr_len = pos - start;
            if (substr_len >= sizeof(tokens[token_count].value)) {
                fprintf(stderr, "Error: Variable token too long.\n");
                return -1;
            }
            strncpy(tokens[token_count].value, expr + start, substr_len);
            tokens[token_count].value[substr_len] = '\0';
        ...
        fprintf(stderr, "Error: Unrecognized character '%c' in expression.\n", expr[pos]);
        return -1;
    }
...
void parse_objectives(Objectives *objectives, char *line) {
    Token tokens[strlen(line)];
    int num_tokens;
    int i;
    char *type;
    char postfix[100] = {0};

    num_tokens = tokenize(line, tokens, 100);
    ...
    for(i = 0; i < num_tokens; i++) {
        switch(tokens[i].type) {
            ...
            case TOKEN_VARIABLE: type = "VARIABLE"; break;
            case TOKEN_OPERATOR: type = "OPERATOR"; break;
            default: type = "UNKNOWN"; break;
        }
        printf("Token %d: Type=%s, Value=%s\n", i+1, type, tokens[i].value);
    }

    shunting_yard(line, postfix);
...
void shunting_yard(const char *infix, char *postfix) {
    ...
    while ((token = infix[i++]) != '\0') {
        ...
        if (isalnum(token) || token == '_') {
            ...
            double top_val;
            while (pop(stack, &top_val) && (char)top_val != '(') {
            ...
            double top_val;
            while (stack->top != -1 && (char)(stack->arr[stack->top]) != '(' &&
                   precedence(token) <= precedence((char)stack->arr[stack->top])) {
                pop(stack, &top_val);
                postfix[k++] = (char)top_val;
                postfix[k++] = ' ';
            }
